...
class CollectorConfig:
    ...
    # 11.09.2026: поднят с 20 до 120 с. Двадцать секунд оказались


    # недостижимы: книга выросла до 442 объявлений на стороне, полный


    # цикл требует ~14 запросов и занимает 30-40 с, циклы накладывались,


    # и эндпоинт долбился непрерывно. Итог — троттлинг: доля неполных


    # обходов росла 0.8% -> 3.2% -> 3.4% -> 9.0% -> 36.6% по суткам,


    # отдельные запросы висели до 12 минут на повторах.
...
class PaperConfig:
    ...
    # Доля РЕАЛЬНОГО оборота контрагента, которую нам позволено занять.
    #
    # Лимит на число сделок не отвечает, много ли для конкретного человека
    # три круга по 300 000. Замерено: для большинства контрагентов наш
    # симулированный оборот составлял 3-20% их настоящего, но у двоих из
    # тридцати четырёх — 141% и 224%. Прокрутить через человека больше,
    # чем он наторговал за всё время наблюдения, физически нельзя.
    #
    # Ноль отключает проверку.
    max_share_of_real_volume: Decimal = field(
        default_factory=lambda: Decimal(os.getenv("PAPER_MAX_REAL_SHARE", "0.5")))
...
class MatchingConfig:
    ...
    # Данные старше этого — не используем.
    #
    # Порог ОБЯЗАН превышать интервал опроса, иначе между двумя обходами
    # вся книга считается протухшей. При подъёме интервала с 20 до 120 с
    # прежние 90 с стали меньше интервала; сейчас фильтр не применяется
    # (никто не передаёт `now` в eligible), поэтому конфликт был тихим —
    # но первый же вызов с `now` отфильтровал бы всё до нуля.
